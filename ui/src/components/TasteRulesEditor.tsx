  ...
  return (
    <div className="oc">
      <div className="oc-selected">
        {selected.length === 0 && <span className="oc-empty">none — add from the tray ↓</span>}
...
function RuleCard({
  ...
  const renderPatternItem = (key: string) => (
    <span className="oc-content">
      ...
      <span className="oc-name">{PATTERN_BY_KEY.get(key)?.name ?? key}</span>
    </span>
  )
  ...
  return (
    <div className="tre-card">
      <div className="tre-card-head">
        ...
        <span className="tre-card-tag">{isDefault ? 'Fallback for any unlabeled part' : SECTION_META[name]?.tag ?? ''}</span>
      </div>

      <div className="tre-row">
        <div className="tre-row-label">Patterns</div>
        <OrderableChips
      ...
      <div className="tre-row">
        <div className="tre-row-label">Palettes</div>
        <OrderableChips
...
// ── randomness phrasing ───────────────────────────────────────────────────────
function randomnessLabel(v: number): string {
  if (v <= 0.15) return 'Predictable — always your top pick'
  if (v <= 0.45) return 'Mostly the favorite, the odd surprise'
  if (v <= 0.75) return 'Balanced variety'
  return 'Wild — a fresh mix every run'
}
    ...
    <div className="tre">
      {!model.ok && (
        <div className="tre-error">
          Couldn’t parse <code>rules.yaml</code> as the expected shape — edit it raw below.
      ...
      <div className="tre-rand">
        <div className="tre-rand-head">
          <span className="tre-rand-title">Variety</span>
          <span className="tre-rand-val">{randomnessLabel(model.randomness)}</span>
        </div>
        <input
      ...
      {model.palettes.length > 0 && (
        <details className="tre-legend">
          <summary>Palette vocabulary · {model.palettes.length}</summary>
      ...
      <div className="tre-hint">
        Click to add · drag to arrange · order = priority (the <b>#1</b> chip is favored when Variety is low).
      </div>
